...
function renderWaitersTable() {
  ...
  if (!waiters.length) {
    body.innerHTML = '<tr><td colspan="6" style="color:var(--muted)">Nuk ka kamarierë. Shtoni të parin më sipër.</td></tr>';
    return;
  }
  body.innerHTML = waiters.map(w => `
    <tr class="${w.active ? "" : "inactive-row"}" data-id="${w.id}">
      ...
      <td>
        ${w.waiter_url
          ? `<div class="link-actions" style="flex-wrap:wrap;gap:0.35rem">
              <input type="text" class="waiter-link-input" readonly value="${escAttr(w.waiter_url)}" style="font-family:monospace;font-size:0.7rem;min-width:140px;flex:1">
      ...
      <td>
        ${w.kds_url
          ? `<div class="link-actions" style="flex-wrap:wrap;gap:0.35rem">
              <input type="text" class="waiter-kds-input" readonly value="${escAttr(w.kds_url)}" style="font-family:monospace;font-size:0.7rem;min-width:140px;flex:1" title="Link për pranimin e porosive (KDS)">
